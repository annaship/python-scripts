...
class Metadata():
  ...
  def get_required_info(self):
    ...
    raw_metadata = self.get_raw_metadata(custom_table_names_list)
    print raw_metadata
  ...
  def get_raw_metadata(self, all_project_ids_str):
    ...
    JOIN `dataset` ON(`required_metadata_info`.`dataset_id` = `dataset`.`dataset_id`) 
    JOIN `project` ON(`dataset`.`project_id` = `project`.`project_id`) 
    JOIN `dna_region` ON(`required_metadata_info`.`dna_region_id` = `dna_region`.`dna_region_id`) 
    JOIN `sequencing_platform` ON(`required_metadata_info`.`sequencing_platform_id` = `sequencing_platform`.`sequencing_platform_id`) 
    JOIN `target_gene` ON(`required_metadata_info`.`target_gene_id` = `target_gene`.`target_gene_id`) 
    JOIN `domain` ON(`required_metadata_info`.`domain_id` = `domain`.`domain_id`) 
    ...
    JOIN `env_package` ON(`required_metadata_info`.`env_package_id` = `env_package`.`env_package_id`) 
    JOIN `run_key` ON(`run_key`.`run_key_id` = `required_metadata_info`.`adapter_sequence_id`) 
    JOIN `illumina_index` ON(`illumina_index`.`illumina_index_id` = `required_metadata_info`.`illumina_index_id`) 
    JOIN `primer_suite` ON(`required_metadata_info`.`primer_suite_id` = `primer_suite`.`primer_suite_id`) 
    JOIN `run` ON(`required_metadata_info`.`run_id` = `run`.`run_id`)
